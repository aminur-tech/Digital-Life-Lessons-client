...
const PublicLessons = () => {
  const { user, role } = useAuth(); // role = "premium" or "free" or undefined
  ...
  const displayedLessons = role === "premium" ? lessons : lessons.slice(0, 6);
    ...
    <div className="max-w-7xl mx-auto px-4 py-10">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
        {displayedLessons.map((lesson) => {
          ...
          // Lock conditions: premium lesson for non-premium users OR private lesson for non-creators
          const locked = (isPremiumLesson && !userIsPremium) || (isPrivateLesson && !isCreator);

          return (
            <div
              key={lesson._id}
              className={`p-5 rounded-xl shadow-lg bg-white relative overflow-hidden ${
                locked ? "opacity-60" : ""
              }`}
            >
              {/* LOCK OVERLAY */}
        ...
        {/* Optional: Show “See More” for free users if there are more than 6 lessons */}
        {role !== "premium" && lessons.length > 6 && (
