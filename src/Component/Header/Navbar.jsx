...
const Navbar = () => {
  ...
  const linkStyle = ({ isActive }) =>
    isActive
      ...
      : 'text-gray-800 dark:text-gray-200 hover:text-blue-500 drop-shadow-sm transition-colors duration-200';
            ...
            {userInfo?.isPremium ? (
              ...
              <NavLink
                ...
                className="relative inline-block px-5 py-2 font-semibold text-black rounded-lg shadow-md text-center
                           bg-yellow-500 hover:bg-yellow-600 overflow-hidden transition-colors"
              >
  ...
  return (
    <div className="navbar sticky top-0 z-50 bg-white/80 dark:bg-gray-900/80 backdrop-blur-lg border-b border-gray-200 dark:border-gray-700 shadow-sm transition-colors">
      {/* LEFT */}
      <div className="navbar-start">
        <div className="dropdown">
          ...
          <ul tabIndex={0} className="menu menu-sm dropdown-content bg-white/70 dark:bg-gray-800/70 backdrop-blur-md rounded-box z-10 mt-3 w-52 p-2 shadow transition-colors">
            {navLinks}
            {!user && (
              <>
      ...
      <div className="navbar-end flex items-center gap-2">
        {!user && (
          <>
            <Link to="/auth/login" className="btn btn-outline rounded-xl">Login</Link>
          ...
          <div className="relative">
            ...
            {dropdownOpen && (
              <div className="absolute right-0 mt-3 w-48 bg-white/80 dark:bg-gray-800/80 backdrop-blur-lg shadow-lg rounded-xl p-3 z-20 transition-colors">
                ...
                {role === "admin" ?
                  <Link className="block py-1 btn w-full" to="dashboard/admin/profile">Profile</Link>
                  :
                  <Link className="block py-1 btn w-full" to="dashboard/profile">Profile</Link>
                }
                <Link className="block py-1 btn w-full" to="/dashboard">Dashboard</Link>
